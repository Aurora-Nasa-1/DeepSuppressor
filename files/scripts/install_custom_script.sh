...
if [ "$ARCH" = "arm64" ]; then
    rm -f "$MODPATH/bin/process_manager_NOAI-x86_64"
    mv "$MODPATH/bin/process_manager_NOAI-aarch64" "$MODPATH/bin/process_manager_NOAI"
fi
if [ "$ARCH" = "x64" ]; then
    rm -f "$MODPATH/bin/process_manager_NOAI-aarch64"
    mv "$MODPATH/bin/process_manager_NOAI-x86_64" "$MODPATH/bin/process_manager_NOAI"
fi
ui_print ""
